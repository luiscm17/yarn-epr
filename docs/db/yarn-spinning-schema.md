...
The 5 Yarn Spinning sections. Fields directly from PRD sección 2.2 and sección 5.2.

| Column | Type | Notes |
|---|---|---|
| `section_id` | integer | PK |
| `section_name` | varchar(50) | Preparation, RingSpinning, Winding, Twisting, Skeining |
| `has_advance` | boolean | false for Winding and Skeining (PRD sección 4.2) |
| `production_recorded_by` | varchar(20) | Quality, Inventory, Unassigned. From PRD sección 2.2 |
| `advance_recorded_by` | varchar(20) | Quality, Inventory, Unassigned. From PRD sección 2.2 |
| `quality_method` | varchar(20) | Samples, BodyKmCuts, RandomCheck. From PRD sección 5.2 |

### 2.5 `machine_groups`

Machine grouping for waste weighing (PRD sección 6.2).
...
| `section_id` | integer | FK → sections.section_id |
| `machine_type` | varchar(10) | PSJ, FIN, or null. From PRD sección 2.2 — needed for FIN-only production, PSJ-only advance |
| `machine_group_id` | integer | FK → machine_groups.machine_group_id (nullable). From PRD sección 6.2 |
...
| `gross_weight_kg` | decimal(8,2) | Full carriage gross weight (PRD sección 3.3) |
| `spindle_count` | integer | Operative spindles for this discharge (PRD sección 3.2) |
| `tare_per_spindle_g` | decimal(6,2) | Spindle/bobbin tare weight in grams (PRD sección 3.2) |
| `carriage_weight_kg` | decimal(8,2) | Empty carriage weight in kg (PRD sección 3.2) |
| `net_weight_kg` | decimal(8,2) | Calculated: gross − (tare_per_spindle × spindles / 1000) − carriage (PRD sección 3.2) |
| `comments` | text | |
...
Skeining production record. No spindles — uses skeins (PRD sección 3.3).
...
| `yarn_count_id` | integer | FK → yarn_counts.yarn_count_id |
| `supervisor_id` | integer | FK → users.user_id |
| `foreman_id` | integer | FK → users.user_id (Inventory, PRD sección 7) |
| `num_skeins` | integer | Number of skeins produced (PRD sección 3.3) |
| `weight_per_skein_g` | decimal(6,2) | Estimated weight per skein. 1 bunch = 10 skeins ~ 5kg (PRD sección 3.3) |
| `net_weight_kg` | decimal(8,2) | Calculated: num_skeins × weight_per_skein_g / 1000 (PRD sección 3.3) |
| `operator_name` | varchar(100) | Free text — operator-dependent production (PRD sección 3.3) |
| `comments` | text | |
...
Advance tracking per machine/turn — summary and process control (PRD sección 4).
Applies to: Preparation (PSJ only), Ring Spinning, Twisting.
Winding and Skeining have no advance (PRD sección 4.2).
...
| `sample_gross_weight_g` | decimal(6,2) | Sample spindle gross weight at turn start (PRD sección 3.2) |
| `sample_tare_weight_g` | decimal(6,2) | That sample spindle tare weight (PRD sección 3.2) |
| `spindle_count` | integer | Total operative spindles (PRD sección 3.2) |
| `weight_input_kg` | decimal(8,2) | Input weight — what the previous turn left as output (PRD sección 4.1) |
| `weight_output_kg` | decimal(8,2) | Output weight — calculated by material balance (PRD sección 4.1) |
| `weight_download_kg` | decimal(8,2) | Sum of all discharge net weights for this turn (PRD sección 4.1) |
| `working_hours` | decimal(4,1) | Hours worked (PRD sección 4.2) |
| `comments` | text | |
| `edit_version` | integer | Default 1 (PRD YS-TR-01) |
| `created_at` | timestamptz | Default now() |
...
Process quality control record (PRD sección 5).
Quality evaluates ALL sections. Method varies by section (PRD sección 5.2).
...
| `section_id` | integer | FK → sections.section_id |
| `yarn_count_id` | integer | FK → yarn_counts.yarn_count_id |
| `inspector_id` | integer | FK → users.user_id (Quality, PRD sección 5.1) |
| `method` | varchar(20) | Samples, BodyKmCuts, RandomCheck (PRD sección 5.2) |
| `sample_values` | jsonb | Array of individual values (Samples method, PRD sección 5.2) |
| `cv_percent` | decimal(5,2) | Coefficient of variation (Samples, PRD sección 5.2) |
| `tenacity` | decimal(5,2) | Tenacity (Samples, PRD sección 5.2) |
| `elongation` | decimal(5,2) | Elongation (Samples, PRD sección 5.2) |
| `body_value` | decimal(8,2) | Body / imperfections (BodyKmCuts, PRD sección 5.2) |
| `kilometers_processed` | decimal(8,2) | Processed km (BodyKmCuts) |
| `cuts_per_cone` | decimal(6,2) | Cuts per cone/bobbin (BodyKmCuts, PRD sección 5.2) |
| `results` | jsonb | Flexible test results. For RandomCheck: the user records whatever they test, with any structure needed (PRD sección 5.3). |
| `comments` | text | |
| `edit_version` | integer | Default 1 (PRD YS-TR-01) |
| `created_at` | timestamptz | Default now() |
...
Waste record by machine group (PRD sección 6).
Inventory registers waste for ALL sections (PRD sección 6.1).
...
| `shift_code` | varchar(1) | FK → shifts.shift_code |
| `date` | date | |
| `weight_kg` | decimal(8,2) | Waste weight in kg (PRD sección 6.2) |
| `waste_type` | varchar(20) | Actual, Accumulated (PRD sección 6.3) |
| `registered_by_id` | integer | FK → users.user_id (Inventory, PRD sección 6.1) |
| `comments` | text | |
| `edit_version` | integer | Default 1 (PRD YS-TR-01) |
| `created_at` | timestamptz | Default now() |
...
2. **Only PSJ have advance in Preparation** (PRD sección 2.2): No `advance_trackings`
   records for Preparation machines where `machines.machine_type != 'PSJ'`.

3. **No advance in Winding or Skeining** (PRD sección 4.2): No `advance_trackings`
...
5. **Advance validates production** (PRD sección 4.4): `weight_download_kg` in
